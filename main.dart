...
class TideColors {
  static const primary  = Color(0xFFFF69B4); // Pink
  static const ink      = Color(0xFF4A3B2A);
  static const inkMuted = Color(0xFF8D7B68);

  static const surface  = Color(0xFFFFF9E6); // Light yellow background
  static const card     = Color(0xFFFFFDF7);

  static const border   = Color(0xFFFFD6E7); // Light pink border

  static const priHigh  = Color(0xFFFF4D8D); // Dark pink
  static const priMed   = Color(0xFFFFD54F); // Yellow
  static const priLow   = Color(0xFFFFB6C1); // Light pink
}
      ...
      body: SafeArea(child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const Text('🌸 Today 💛', style: TextStyle(
            fontSize: 28, fontWeight: FontWeight.w800, color: TideColors.ink)),
          Text('$remaining tasks remaining',
            style: const TextStyle(color: TideColors.inkMuted)),
          const SizedBox(height: 24),
          Expanded(child: ListView(
            children: tasks.map((t) =>
              TaskCard(task: t, onTap: () => _toggle(t))).toList(),
          )),
        ]),
      )),
      floatingActionButton: FloatingActionButton(
        onPressed: _openSheet, backgroundColor: TideColors.primary,
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(mainAxisSize: MainAxisSize.min,
      ...
      TextField(controller: _c, autofocus: true,
        decoration: InputDecoration(hintText: 'What needs doing?',
          border: OutlineInputBorder(
            borderRadius: BorderRadius.circular(12)))),
      const SizedBox(height: 16),
      Row(children: ['high', 'med', 'low'].map((p) {
        final sel = _priority == p;
        return Padding(padding: const EdgeInsets.only(right: 8),
          child: ChoiceChip(label: Text(p.toUpperCase()),
            selected: sel, selectedColor: TideColors.primary,
